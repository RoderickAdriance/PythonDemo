...
def construct_feature_columns():
    ...
    print(boundaries)
    bucketized_households = tf.feature_column.bucketized_column(households, boundaries=boundaries)

    # 对 经度 分桶
    boundaries = get_quantile_based_boundaries(training_examples["longitude"], 10)
    print(boundaries)
    bucketized_longitude = tf.feature_column.bucketized_column(longitude, boundaries=boundaries)

    # 对 纬度 分桶
    boundaries = get_quantile_based_boundaries(training_examples["latitude"], 10)
    bucketized_latitude = tf.feature_column.bucketized_column(latitude, boundaries=boundaries)

    # 对 房子 年龄 分 桶
    boundaries = get_quantile_based_boundaries(training_examples["housing_median_age"], 7)
    bucketized_housing_median_age = tf.feature_column.bucketized_column(housing_median_age, boundaries=boundaries)

    # 对 平均收入分桶
    boundaries = get_quantile_based_boundaries(training_examples["median_income"], 7)
    bucketized_median_income = tf.feature_column.bucketized_column(median_income, boundaries=boundaries)

    # 对 人均房间分桶
    boundaries = get_quantile_based_boundaries(training_examples["rooms_per_person"], 7)
    bucketized_rooms_per_person = tf.feature_column.bucketized_column(rooms_per_person, boundaries=boundaries)
